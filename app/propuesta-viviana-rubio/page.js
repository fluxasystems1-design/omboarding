...
const SITE_URL = "https://wa.link/chsnoe";
const INSTAGRAM_URL = "https://www.instagram.com/nutricionista.vivianarubio/";
const NUTRIXION_URL = "https://www.instagram.com/nutrixionfuncional/";
...
const FICHA = [
  { label: "Instagram", value: "@nutricionista.vivianarubio" },
  { label: "Seguidores", value: "837 seguidores y 14 publicaciones" },
  { label: "Otros roles", value: "Línea @nutrixionfuncional" },
  { label: "Sitio web", value: "Sin homepage propia · WhatsApp wa.link/chsnoe" },
  { label: "Especialidad", value: "Nutrición Funcional Ortomolecular · Metabolismo · Terapias Alternativas" },
  { label: "Modalidad", value: "Consultas en Ibagué y online" },
  { label: "Producto validado", value: "Suplementos nutraceúticos @nutrixionfuncional + consulta" },
];

const ASSETS = [
  "Cuenta con 837 seguidores y contenido activo en nutrición funcional",
  "Producto ya en mercado: suplementos nutraceúticos @nutrixionfuncional",
  "Contenido orientado a empresarios y bienestar (estrés, ansiedad, zona de confort)",
  "Línea de suplementos @nutrixionfuncional: canal de producto sin embudo digital",
  "Bio con enfoque claro: nutrición funcional + metabolismo + terapias alternativas",
];

const FRICTION_CARDS = [
  {
    title: "Sin homepage profesional de conversión",
    text: "Hoy el camino es Instagram y WhatsApp. No hay una landing con jerarquía clara, testimonios ni checkout. Todo termina en mensaje 1:1, el modelo menos escalable.",
  },
  {
  ...
  {
    title: "Producto validado pero aislado",
    text: "Los suplementos y la consulta viven separados: no hay escalera de productos ni entrada a algo recurrente.",
  },
  {
    title: "Contenido sin método propio bautizado",
    text: 'El contenido retiene atención, pero no apunta a un sistema con nombre que sea "lo que vendes".',
  },
  {
    title: "Nutrixion Funcional desconectado del funnel",
    text: "Línea de suplementos sin puente claro hacia la venta digital y la membresía.",
  },
  {
...
const TRANSFORMATIONS = [
  {
    before: "Sin homepage profesional, sin jerarquía ni optimización",
  ...
  {
    before: "Nutrixion Funcional desconectado",
...
const JOURNEY_STEPS = [
  ...
  },
  {
    step: "Fase 4",
    title: "Escala",
    text: "Alto ticket + Nutrixion Funcional integrado al funnel.",
    tag: "Desde mes 3",
  },
];
...
const DIGITAL_SECTIONS = [
  {
    ...
    items: [
      "Landing de marca que reemplaza la presencia actual, con checkout simple para el producto digital",
...
const PRO_EXTRA_SECTIONS = [
  {
    ...
    items: [
      ...
      "Conexión de Nutrixion Funcional al funnel",
    ],
  },
  {
...
const FLOATING_FOODS = [
  { emoji: "🥑", className: "viv-food--1" },
  { emoji: "🥗", className: "viv-food--2" },
  { emoji: "🫐", className: "viv-food--3" },
  { emoji: "🥦", className: "viv-food--4" },
  { emoji: "🍋", className: "viv-food--5" },
  { emoji: "🍓", className: "viv-food--6" },
  { emoji: "🌿", className: "viv-food--7" },
  { emoji: "🥕", className: "viv-food--8" },
  { emoji: "🫐", className: "viv-food--9" },
  { emoji: "🥬", className: "viv-food--10" },
];

function FoodAmbient() {
  return (
    <div className="viv-food-ambient" aria-hidden>
      {FLOATING_FOODS.map((item) => (
        <span key={item.className} className={`viv-food ${item.className}`}>
          <span className="viv-food__emoji">{item.emoji}</span>
        </span>
      ))}
    </div>
  );
...
export default function PropuestaVivianaRubioPage() {
    ...
    <main className="mafe-page">
      ...
      <section id="hero" className="mafe-hero-mesh scroll-mt-28 pb-16 pt-16 lg:pb-20 lg:pt-24">
        ...
        >
          ...
          <h1 className="mafe-hero-brand">Viviana Rubio</h1>
        ...
        <div data-reveal className="mafe-reveal mafe-hero-band relative z-[2] mt-14 w-full">
          <div className="mx-auto flex max-w-6xl flex-col items-center px-4 py-10 text-center sm:px-6 sm:py-12">
            <p className="mafe-hero-band-handle text-lg font-semibold tracking-wide sm:text-xl">
              @nutricionista.vivianarubio
            </p>
            <p className="mt-2 text-sm text-white/75">Nutricionista Funcional Ortomolecular · Esp. Terapias Alternativas</p>
          </div>
        </div>
      ...
      <div className="viv-body">
        <FoodAmbient />
        <div className="viv-body-content">
      <SectionBlock
        id="donde"
        eyebrow="01. Punto de partida"
        title="Tienes comunidad, especialidad clara y producto en marcha."
      ...
      >
        ...
        <div data-reveal className="mafe-reveal mafe-callout mt-8 rounded-xl p-5 sm:p-6">
          ...
          <p className="mafe-card-text mt-3 text-sm leading-relaxed sm:text-base">
            ...
            con botones de conversión, cupos limitados como escasez real. Replicamos ese mecanismo, no su ángulo de
            pérdida de peso/estética, incompatible con el posicionamiento de nutrición funcional y metabolismo de
            Viviana Rubio.
          </p>
        </div>
      </SectionBlock>
        ...
        <div className="mafe-stagger-group grid gap-4 md:grid-cols-2" data-reveal>
          <article className="mafe-card mafe-stagger rounded-xl p-6" style={staggerStyle(0)}>
            ...
            <ul className="mafe-muted mt-4 space-y-2 text-sm leading-relaxed">
              ...
              <li>Nutrixion Funcional sin conexión al negocio</li>
            </ul>
          </article>
          <article className="mafe-card mafe-card--featured mafe-stagger rounded-xl p-6" style={staggerStyle(1)}>
            ...
            <ul className="mafe-card-text mt-4 space-y-2 text-sm leading-relaxed">
              ...
              <li>Membresia con cobro automático y Nutrixion Funcional integrado al funnel</li>
            </ul>
          </article>
        </div>
      ...
      <footer className="px-4 pb-24 pt-2 text-center text-[11px] uppercase tracking-[0.16em] text-[var(--mafe-muted-light)] sm:px-6">
        ...
        {" | "}
        <a href={NUTRIXION_URL} target="_blank" rel="noopener noreferrer" className="hover:underline">
          Nutrixion Funcional
        </a>
        {" | Fluxa Method"}
      </footer>
